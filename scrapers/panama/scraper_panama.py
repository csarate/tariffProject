...
def arma_csv(link, path, date_ori, local):
    ...
    nombre = "panama_tariff_asep_" + anio_mes
    nombre = f"{nombre}.csv"
...
# -------------------------------------------
def panama_asep_tariff(time_to_search, output_path, event: dict, local):
    ...
    date_limit = "2012"
    sl._create_default_https_context = sl._create_unverified_context
    ...
    try:
        ...
        try:
            ...
            while (time.time() - beginning_time < time_to_search) or last_iteration:
                ...
                valor, soup = util.status_code_url(s, base_link, local)
